...
import { Headphones, Mic, Eye, PenTool } from "lucide-react";
          ...
          {skills.map((skill, index) => (
            ...
            >
              <CardContent className="p-8">
                <div className={`w-16 h-16 rounded-2xl ${skill.color} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}>
                  {(skill as any).customLogo ? (
                    <img src={(skill as any).customLogo} alt={skill.title} className="w-10 h-10" />
                  ) : (
